import { utf8ByteArrayToString } from "./crypt.js";
import { getNumUnit } from "./changeUnit.js";
import { formatDate2 } from "./util.js";
...
const toTable108 = function(dataView) {
  ...
  data.type = "108";
  data.code =
    addZero(dataView.getInt16(2), 3) + addZero(dataView.getInt16(4), 3);
...
const toTable112 = function(dataView) {
  ...
  for (let i = 0; i < itemLength; i++) {
    ...
    temp.dealPrice = dataView
      .getFloat32(i * dataByteLength + order + 4)
      .toFixed(2);
...
const toStockTable = function(stockTableView) {
  ...
  data.type = "105";
  data.code =
    addZero(stockTableView.getInt16(2), 3) +
    addZero(stockTableView.getInt16(4), 3);
...
const toAbnormalTable = function(dataView) {
  ...
  data.totalPage = addZero(dataView.getInt16(14), 3);
  data.page = addZero(dataView.getInt16(16), 3);
  data.sortCode = addZero(
    "" + dataView.getInt16(18) + dataView.getInt16(20),
    4
  );
  data.data = [];
  let dataByteLength;
  ...
  for (let i = 0; i < itemLength; i++) {
    ...
    temp.lastTrade = dataView
      .getFloat32(i * dataByteLength + 32 + 4)
      .toFixed(2);
    temp.rise = (
      dataView.getFloat32(i * dataByteLength + 32 + 8) * 100
    ).toFixed(2);
    temp.hand = Math.round(dataView.getFloat32(i * dataByteLength + 32 + 12));
    temp.keyValue = dataView
      .getFloat32(i * dataByteLength + 32 + 16)
      .toFixed(2);
    let strArr = [];
    for (let j = 20; j < 32; j++) {
...
const to106Table = stockTableView => {
  ...
  while (true) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      childObj.cname = utf8ByteArrayToString(childStrArr).replace(
        /\u0000/g,
        ""
      );
      childStrArr = [];
      ...
      childObj.keyName = utf8ByteArrayToString(childStrArr).replace(
        /\u0000/g,
        ""
      );
...
const toTable145 = function(stockTableView) {
  ...
  while (stockTableView.byteLength > 32) {
    ...
    obj.rise =
      ((parseFloat(obj.cj) / parseFloat(obj.zsj)) * 100).toFixed(2) + "%";
    for (let i = 0; i < 17; i++) {
      strArr.push(stockTableView.getUint8(order));
      order += 1;
    }
