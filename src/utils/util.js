...
const request = (
  ...
  return new Promise((resolve, reject) => {
    ...
    if (needEncrypt === true) {
      ...
      let signature = sha1.hex_sha1(
        wx.getStorageSync("token") + random + timestamp
      );
                              ...
                              wx.setStorage({
                                ...
                                success: () => {
                                  if (needEncrypt === true) {
                                    ...
                                    hearderObj["signature"] = signature;
                                    hearderObj["uid"] = wx.getStorageSync(
                                      "uid"
                                    );
                                  }
                                  wx.request({
...
const formatTimeYmd = oldTime => {
  // 将毫秒时间戳转化为年月日（2001-01-01）
  let newTime = new Date(oldTime);
  let ytime = newTime.getFullYear() + "-";
  let mtime =
    (newTime.getMonth() + 1 < 10 ? "0" : "") + (newTime.getMonth() + 1) + "-";
